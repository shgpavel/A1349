...
\begin{frame}{Mapping to sched\_ext}
        \begin{columns}
                ...
                \textbf{enqueue:}
                $$VE_i = \max(v_i,\; V_\mathrm{now} - \Delta),\quad v_i \leftarrow VE_i$$
...
\begin{frame}{Extra}
        ...
        \begin{itemize}
                \item In the continuous model, a sleeping task accumulates
                        negative lag $-\tau$ for virtual time $\tau$ it was absent
                ...
                \item The clamp enforces $VE_i \ge V_{\mathrm{now}} - \Delta$,
                        so at most one quantum $\Delta$ of credit is carried forward
                \item $\mathrm{lag}_i = \text{(ideal service)} -
                        \text{(actual service)} \le \Delta$ at the moment of
                        re-enqueue, which is exactly the bound $|\mathrm{lag}_i| < q$
                        with $q = \Delta$
        \end{itemize}
...
\begin{frame}{Extra}
       \textbf{Critiques:} (a) lazy advancement breaks the $V(t)$ definition;
        ...
        \begin{itemize}
                ...
                \item \texttt{vtime\_now} $\leftarrow \max(\texttt{vtime\_now},\; v_i)$
                        in \texttt{running()} is exactly such a monotone frontier
        ...
        \vspace{0.15cm}
        \textbf{(b) \texttt{total\_weight} is used -- in \texttt{set\_weight()}:}
        \begin{itemize}
                \item When a task's weight changes, the lag must be
                        rescaled: $\mathrm{lag} \cdot W_{\mathrm{old}} / W_{\mathrm{new}}$
                \item The code adjusts \texttt{vtime\_now} by exactly this
                        proportional correction, preserving relative virtual positions
...
\begin{frame}{Extra}
      \textbf{Critique:} $S$ appears without justification;
        ...
        \vspace{0.3cm}
        \textbf{Precision analysis} (sched\_ext weight range: $1 \le w \le 10000$;
        ...
        ratio $\Delta \cdot S / w_i$ matters, and it is exact integer arithmetic}
\end{frame}
\addtocounter{framenumber}{-1}
...
\begin{frame}{Extra}
        ...
        \begin{itemize}
                ...
                \item sched\_ext uses a fixed $\Delta = \texttt{SCX\_SLICE\_DFL} = 5\,\text{ms}$
                ...
                \item Sysbench is a single-threaded CPU-bound workload;
                        ...
                        its default shorter quanta and context-switches more often
                \item Fewer preemptions $\Rightarrow$ lower context-switch overhead
        ...
        \begin{shadowbox2}
                \footnotesize{The schedulers implement the same EEVDF \emph{policy} and
                the throughput gap traces to a \emph{parameter} difference,
