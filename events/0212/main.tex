...
\begin{frame}
  ...
  \begin{block}{Задачи}
    \begin{enumerate}
      ...
      \item Создать тестовое окружение и набор тестовых программ
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{equation}
        ...
        \tag{ideal minus actual service time}
    \end{equation}
    
    \column{0.5\textwidth}
    \begin{equation}
        V(t) = \int_{t_0}^{t} \frac{1}{\sum_{j \in A(\tau)} w_j} d\tau
        \tag{system virtual time}
    \end{equation}

    \begin{equation}
        S_i(t_1, t_2) = w_i (V(t_2) - V(t_1))        
        \tag{ideal service time}
    \end{equation}

    \begin{equation}
        V(e) = V(t_0) + \frac{s_i(t_0, t)}{w_i}
        \tag{virtual eligible time}
    \end{equation}
  \end{columns}
\end{frame}
