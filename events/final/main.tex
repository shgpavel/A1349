...
\begin{frame}{sched\_ext}
        \vspace{-0.6em}
        \begin{shadowbox4}
                \footnotesize \textbf{sched\_ext} (scheduler extensions)~---
                инфраструктура ядра Linux, позволяющая реализовывать
                политику планирования как eBPF-программу,
                подключаемую к интерфейсу расширяемых планировщиков без пересборки ядра
        \end{shadowbox4}
        \vspace{-0.6em}
        ...
        \begin{columns}[T]
                ...
                \centering
                \includegraphics[width=\linewidth,height=0.67\textheight,keepaspectratio]{res/sched_ext.pdf}\\[-0.8em]
                {\tiny \textbf{Рис.~1.} Механизм работы sched\_ext}

                \column{0.26\textwidth}
                \vspace{0.7em}
                \textbf{\footnotesize DSQ} {\scriptsize(dispatch queue)}
                {\scriptsize
                        \begin{itemize}\setlength{\itemsep}{0pt}
                                \item \texttt{SCX\_DSQ\_LOCAL}
                                \item \texttt{SCX\_DSQ\_GLOBAL}
                                \item пользовательские DSQ
                \end{itemize}}

                \vspace{0.2em}
                \textbf{\footnotesize Цикл планирования}
                {\scriptsize
                        \begin{itemize}\setlength{\itemsep}{0pt}
                                \item \texttt{select\_cpu}
                                \item \texttt{enqueue}
                                \item \texttt{dispatch}
...
\begin{frame}{Выводы}
        \begin{itemize}\setlength{\itemsep}{0.4em}
                \item Распределение слотов P/E\nobreakdash-ядер
                        формализовано как динамический аукцион VCG:
                        состояние, действие, эффективная ценность
                        $\phi_\kappa$, платёж по pivot\nobreakdash-правилу.
                        Обоснованы стимульная совместимость в
                        пределах класса, индивидуальная рациональность
                        и бюджетное ограничение
                \item Механизм реализован как BPF\nobreakdash-планировщик
                        \texttt{scx\_auction} поверх \texttt{sched\_ext}
                        (DSQ\_P / DSQ\_E / DSQ\_Starved, маршрутизация
                        по $\rho_i$, прирост бюджета за idle как аналог
                        порогового правила Майерсона)
                \item На Intel Core Ultra 7 265K (8P+12E) получено
                        лучшее время hackbench (8.43\,s, $-9\%$ к
                        Linux~EEVDF) и наименьшая средняя длительность
                        сна (26\,ms). Sysbench и энергопотребление
                        сопоставимы, p99 sched\nobreakdash-delay
                        проигрывает scx\_LAVD~-- очередное направление
                        работы
        \end{itemize}
\end{frame}
